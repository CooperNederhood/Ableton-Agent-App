 ...
 * precede the Live Set rows the application has observed.
 */
const projectionsAndConstraints: ProjectStateMigration = {
  version: 2,
  description: "Add lookup projections, indexes, and unique constraints",
  statements: [
    `ALTER TABLE sessions ADD COLUMN active_live_set_id TEXT`,
    `ALTER TABLE sessions ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''`,
    `UPDATE sessions SET
      active_live_set_id = json_extract(payload, '$.activeLiveSetId'),
      updated_at = COALESCE(json_extract(payload, '$.updatedAt'), '')`,
    `ALTER TABLE live_sets ADD COLUMN last_seen_at TEXT NOT NULL DEFAULT ''`,
    `UPDATE live_sets SET
    ...
    `UPDATE preferences SET
      updated_at = COALESCE(json_extract(payload, '$.updatedAt'), '')`,
    `ALTER TABLE approvals ADD COLUMN live_set_id TEXT NOT NULL DEFAULT ''`,
    ...
    `UPDATE approvals SET
      live_set_id = COALESCE(json_extract(payload, '$.liveSetId'), ''),
      session_id = COALESCE(json_extract(payload, '$.sessionId'), ''),
      decided_at = COALESCE(json_extract(payload, '$.decidedAt'), '')`,
    `CREATE UNIQUE INDEX live_sets_live_set_id
      ON live_sets (live_set_id)`,
    `CREATE INDEX sessions_active_live_set_id
      ON sessions (active_live_set_id)`,
    `CREATE INDEX plans_live_set_id ON plans (live_set_id, created_at, id)`,
    `CREATE UNIQUE INDEX change_sets_correlation_id
      ON change_sets (correlation_id)`,
    `CREATE INDEX change_sets_live_set_id
      ON change_sets (live_set_id, created_at, id)`,
    ...
    `CREATE INDEX approvals_live_set_id ON approvals (live_set_id)`,
  ],
};
